	...
	{	case VIEW_ASIGN :
			 ...
			 <table class='display' id='tabla_asigna_repr' width='100%'>
				<thead>
					<tr>       
						...
						<th style='text-align:center;font-size:11px;'>Representante Legal</th>
						<th style='text-align:center;font-size:11px;'>Titular facturas</th>
						<th style='text-align:center;font-size:11px;'>Eliminar</th>
					</tr>
				</thead>";
			...
			{	if($c<($aux-1))
				{	$body.="<tr>";
					...
					$opc_eliminar = "<span onclick='js_representantes_delete_repr_relacion(\"".$alum_codi."\",\"".$repr_codi."\",".'"'.$diccionario['rutas_head']['ruta_html'].'/representantes/controller.php"'.")' class='btn_opc_lista_eliminar glyphicon glyphicon-trash cursorlink' aria-hidden='true' id='".$repr_codi."_eliminar' onmouseover='$(this).tooltip(".'"show"'.")' data-placement='left' title='Eliminar relación'></span>";
					
					$body.= "<td style='text-align:center'>".$radioMain."</td>";
					$body.= "<td style='text-align:center'>".$radioFina."</td>";
					$body.= "<td style='text-align:center'>".$opc_eliminar."</td>";
				}
				$body.="</tr>";
				$c++;
			}////////js_representantes_delete_repr_relacion
		...
		case UPD_FACTURAS_POR_AUTORIZAR :
			...
			retornar_result($data);
			break;
		case DELETE_REPR_RELACION :
			$resultado = $representante->delete_repr_relacion($user_data['alum_codi'],$user_data['repr_codi']);
			$data =array("mensaje" => $resultado->mensaje);
			retornar_result($data);
			break;
        default :
